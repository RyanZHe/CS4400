...
###################################################### Student Functionalities
    ...
    def searchProject(self):
        ...
        if designation != '':
            if numconstraints == 0:
                ...
                sql = sql + " AND"
            sql = sql + " Project.Designation_name = \'%s\'" % designation
        if major != '':
            if numconstraints == 0:
                sql = sql + " WHERE"
                numconstraints = 1
            ...
            if department != '':
                if numconstraints == 0:
                    ...
                    sql = sql + " OR"
                sql = sql + " Project_requirement.Requirement LIKE '%%%s%%'" % department
        if year != '':
            if numconstraints == 0:
                sql = sql + " WHERE"
                numconstraints = 1
    ...
    def viewProject(self, name):
        ...
        Label(f, text = self.connect("SELECT NumStudent FROM Project WHERE Name = \'%s\'" % name, "Return Integer"), font=("Helvetica", 20)).grid(row = 0, column = 1)
        f.pack()
    ...
    def applyProject(self, name):
        ...
        year = self.dYear.get()
        department = self.connect("SELECT Dept_Name FROM Major WHERE Name = \'%s\'" % major, "Return Single Item")[0]
        if len(self.connect("SELECT * FROM Project_requirement WHERE Type = 'Major' AND Name = \'%s\'" % name, "Return Single Item")) != 0:
            if len(self.connect("SELECT * FROM Project_requirement WHERE Requirement LIKE '%%%s%%' AND Type = 'Major' AND Name = \'%s\'" % (major, name), "Return Single Item")) == 0:
                error = messagebox.showerror("Error", "Your major does not fulfill the requirement")
                return
        if len(self.connect("SELECT * FROM Project_requirement WHERE Type = 'Year' AND Name = \'%s\'" % name, "Return Single Item")) != 0:
            if len(self.connect("SELECT * FROM Project_requirement WHERE Requirement LIKE '%%%s%%' AND Type = 'Year' AND Name = \'%s\'" % (year, name), "Return Single Item")) == 0:
                error = messagebox.showerror("Error", "Your year does not fulfill the requirement")
                return
        if len(self.connect("SELECT * FROM Project_requirement WHERE Type = 'Department' AND Name = \'%s\'" % name, "Return Single Item")) != 0:
            if len(self.connect("SELECT * FROM Project_requirement WHERE Requirement LIKE '%%%s%%' AND Type = 'Department' AND Name = \'%s\'" % (department, name), "Return Single Item")) == 0:
        ...
        try:
            ...
            error = messagebox.showerror("Error", "You have already applied to the project")
            self.returnTo(self.viewProjectWin)
    ...
    def searchCourse(self):
        ...
        if designation != '':
            if numconstraints == 0:
                ...
                sql = sql + " AND"
            sql = sql + " Course.Designation_name = \'%s\'" % designation
        if len(icategorySet) != 0:
            for category in icategorySet:
                if numconstraints == 0:
                    sql = sql + " WHERE"
    ...
    def viewCourse(self, name):
        ...
        f = Frame(self.viewCourseWin)
        Label(f, text = "Estimated number of students: ", font=("Helvetica", 20)).grid(row = 0, column = 0)
        Label(f, text = self.connect("SELECT %s FROM Course WHERE Name = \'%s\'" % ("NumStudent", name), "Return Integer"), font=("Helvetica", 20)).grid(row = 0, column = 1)
        f.pack()
...
###################################################### Admin Functionalities
    ...
    def viewApplications(self):
        ...
        for num in range(1,int(numProj[0][0]) + 1):
            ...
            if self.currentStatus == 'Pending':
                ...
                name = projects[num - 1][2]
                projectname = projects[num - 1][0]
                # OPTIONS = ['Accept', 'Pending', 'Reject']
                # self.dProject = StringVar()
                # self.dProject.set("Pending")
                # dropdown = OptionMenu(f, self.dProject, *OPTIONS, command = self.callback)
                # dropdown.config(width = 10)
                # dropdown.grid(row = num, column = 3)
                Button(f, text = "Accept", command = lambda name = name, projectname = projectname: self.acceptA(projectname, name)).grid(row = num, column = 3)
                Label(f, text = "Pending").grid(row = num, column = 4)
                Button(f, text = "Reject", command = lambda name = name, projectname = projectname: self.rejectA(projectname, name)).grid(row = num, column = 5)
    ...
    def callback(self, value):
        if value != 'Pending':
            ...
            if value == 'Accept':
                sql = "UPDATE Apply SET Status = \'Accepted\' WHERE Project_name = \'%s\' AND Student_name = \'%s\'" % (projectName, name)
            elif value == 'Reject':
                sql = "UPDATE Apply SET Status = \'Rejected\' WHERE Project_name = \'%s\' AND Student_name = \'%s\'" % (projectName, name)
            else:
                sql = "UPDATE Apply SET Status = \'Pending\' WHERE Project_name = \'%s\' AND Student_name = \'%s\'" % (projectName, name)
    ...
    def acceptA(self, projectName, name):
        self.acceptAWin = Toplevel(self.loginWin)
        self.acceptAWin.geometry('{}x{}'.format(300, 200))
        self.acceptAWin.title("Change Status")
        sql = "UPDATE Apply SET Status = \'Accepted\' WHERE Project_name = \'%s\' AND Student_name = \'%s\'" % (projectName, name)
        f = Frame(self.acceptAWin)
        f.pack()
        Label(f, text = "Are you sure? No changes can be done.").grid(row = 0, column = 0, columnspan = 2)
        Button(f, text = "Yes", command = lambda: self.changeStatus1(sql)).grid(row = 1, column = 0)
        Button(f, text = "No", command = self.chooseFunctionality).grid(row = 1, column = 1)

    def rejectA(self, projectName, name):
        self.rejectAWin = Toplevel(self.loginWin)
        self.rejectAWin.geometry('{}x{}'.format(300, 200))
        self.rejectAWin.title("Change Status")
        sql = "UPDATE Apply SET Status = \'Rejected\' WHERE Project_name = \'%s\' AND Student_name = \'%s\'" % (projectName, name)
        f = Frame(self.rejectAWin)
        f.pack()
        Label(f, text = "Are you sure? No changes can be done.").grid(row = 0, column = 0, columnspan = 2)
        Button(f, text = "Yes", command = lambda: self.changeStatus2(sql)).grid(row = 1, column = 0)
        Button(f, text = "No", command = self.chooseFunctionality).grid(row = 1, column = 1)

    def changeStatus1(self, sql):
        self.viewAppsWin.withdraw()
        self.acceptAWin.withdraw()
        self.connect(sql, "Insertion")
        self.viewApplications

    def changeStatus2(self, sql):
        self.viewAppsWin.withdraw()
        self.rejectAWin.withdraw()
        self.connect(sql, "Insertion")
        self.viewApplications
    ...
    def viewPopularProjectReport(self):
        ...
        # RIGHT OUTER JOIN Project ON Apply.Project_name = Project.Name)
        sql = "SELECT Project_name, COUNT(Student_name) AS Number_of_Students FROM Apply GROUP BY Project_name ORDER BY Number_of_Students DESC"
        ...
        for i in range(1, length):
            Label(f, text = popProjectList[i-1][0]).grid(row = i, column = 0, sticky = W)
            Label(f, text = popProjectList[i-1][1]).grid(row = i, column = 1)
...
###################################################### Connect to Database
    ...
    def connect(self, sql, commandName, parameters = None):
        ...
        try:
            ...
            elif commandName == "Return Integer":
                print(sql)
                cursor.execute(sql)
